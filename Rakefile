...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/mattbrictson/tomo-plugin-nvm/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("tomo-plugin-nvm.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in tomo-plugin-nvm.gemspec include the
